        ...
        <div className="w-[70px] px-2 py-1 text-right relative" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-1 bg-[#1e222d] border border-[#2a2e39] rounded-lg shadow-lg z-10 min-w-[80px]">
              <button
                onClick={() => { setTimeframe("intraday"); setDropdownOpen(false); }}
                className={`w-full text-left px-3 py-1.5 text-xs hover:bg-[#2a2e39] transition-colors ${timeframe === "intraday" ? "text-[#2962ff]" : "text-[#d1d4dc]"}`}
              >
                Intraday
              </button>
              <button
                onClick={() => { setTimeframe("daily"); setDropdownOpen(false); }}
                className={`w-full text-left px-3 py-1.5 text-xs hover:bg-[#2a2e39] transition-colors ${timeframe === "daily" ? "text-[#2962ff]" : "text-[#d1d4dc]"}`}
              >
                Daily
              </button>
            </div>
            ...
            {sortedData.map((row, index) => (
              ...
              >
                <td className="w-[60px] py-1 px-3">
                  <a
                    href={`https://analytics.tsis.ai/ticker/${row.ticker}`}
                    target="_blank"
                ...
                <td className={`w-[70px] py-1 px-3 text-right ${row.change >= 0 ? "text-[#26a69a]" : "text-[#ef5350]"}`}>
