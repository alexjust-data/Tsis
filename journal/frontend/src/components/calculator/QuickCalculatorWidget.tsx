...
function RiskBadge({ status, message }: { status: RiskStatus; message?: string }) {
  const config = {
    GREEN: { bg: "bg-[#26a69a]", text: "RISK OK" },
    ORANGE: { bg: "bg-[#f59e0b]", text: "ALERT" },
    RED: { bg: "bg-[#ef5350]", text: "STOP" },
  };
...
export default function QuickCalculatorWidget() {
  ...
  return (
    <div className="bg-[#131722] rounded border border-[#2a2e39] h-full flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#2a2e39]">
        <div className="flex items-center gap-2">
          <Zap className="h-5 w-5 text-[#f59e0b]" />
          <h3 className="text-[15px] font-bold text-white">Risk Management</h3>
        </div>
        <button
          onClick={openModal}
          className="flex items-center gap-1.5 px-2 py-1 text-[11px] text-[#787b86] hover:text-white hover:bg-[#2a2e39] rounded transition-colors"
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="flex gap-3 mb-3">
          <div className="flex-1">
            <label className="block text-[11px] text-[#787b86] mb-1.5 uppercase tracking-wide">Ticker</label>
            <input
              type="text"
              value={ticker}
          ...
          <div className="w-28">
            <label className="block text-[11px] text-[#787b86] mb-1.5 uppercase tracking-wide">Side</label>
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1">
            <label className="block text-[11px] text-[#787b86] mb-1.5 uppercase tracking-wide">Entry</label>
            <input
              type="number"
              step="0.01"
              value={entryPrice}
          ...
          <div className="flex-1">
            <label className="block text-[11px] text-[#787b86] mb-1.5 uppercase tracking-wide">Stop</label>
            <input
              type="number"
              step="0.01"
              value={stopPrice}
        ...
        {/* Divider */}
        <div className="border-t border-[#2a2e39] mb-4" />
        ...
        <div className="flex-1 flex flex-col justify-center">
          {lastResult ? (
            <>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-[13px] text-[#787b86]">SHARES</span>
                  </div>
                </div>
                <button
                  onClick={handleCopy}
              ...
              {/* Risk Info */}
              <div className="text-[13px] text-[#787b86] mb-3">
            ...
            <>
              ...
              <div className="text-center py-4">
                <div className="text-[13px] text-[#787b86] mb-1">Shares</div>
                <div className="text-3xl font-bold text-[#363a45]">—</div>
                <div className="text-[12px] text-[#787b86] mt-1">Risk: —</div>
              </div>

              {/* Risk Status Placeholder */}
              <div className="flex items-center gap-2 px-3 py-2 rounded bg-[#2a2e39]/50 border border-[#2a2e39]">
                <span className="w-2 h-2 rounded-full bg-[#787b86]" />
                <span className="text-[12px] text-[#787b86]">Enter values to calculate</span>
              </div>
            </>
          )}
        </div>
        ...
        <button
          onClick={openModal}
          className="mt-4 w-full py-2.5 bg-[#2a2e39] hover:bg-[#363a45] text-white text-[13px] font-medium rounded transition-colors flex items-center justify-center gap-2"
        >
