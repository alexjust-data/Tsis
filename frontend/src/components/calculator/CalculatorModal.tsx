...
export default function CalculatorModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-2xl mx-4 bg-[#14161d] border border-[#2d3139] rounded-lg shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2d3139]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[#00a449]/20 flex items-center justify-center">
              <Calculator className="h-5 w-5 text-[#00a449]" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-white">Calculadora Rápida</h2>
              <p className="text-[12px] text-[#707990]">F2 para abrir/cerrar | ESC para cerrar</p>
            </div>
          </div>
          <button
            onClick={closeModal}
            className="p-2 text-[#707990] hover:text-white hover:bg-[#2d3139] rounded transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            {/* Ticker */}
            <div>
              <label className="block text-[12px] text-[#707990] mb-1.5">TICKER</label>
              <input
                ref={tickerRef}
                type="text"
                value={ticker}
            ...
            {/* Entry Price */}
            <div>
              <label className="block text-[12px] text-[#707990] mb-1.5">ENTRY</label>
              <input
                type="number"
                step="0.01"
                value={entryPrice}
            ...
            {/* Stop Price */}
            <div>
              <label className="block text-[12px] text-[#707990] mb-1.5">STOP</label>
              <input
                type="number"
                step="0.01"
                value={stopPrice}
            ...
            {/* Side Toggle */}
            <div>
              <label className="block text-[12px] text-[#707990] mb-1.5">SIDE</label>
          ...
          {/* Result Box */}
          {lastResult && (
            <div className="bg-[#22262f] border border-[#2d3139] rounded-lg p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-[#707990] text-lg">SHARES</span>
                  </div>
                </div>
              ...
              {/* Details Grid */}
              <div className="grid grid-cols-4 gap-4 pt-4 border-t border-[#2d3139]">
                <div>
                  <p className="text-[11px] text-[#707990] mb-1">Exposición</p>
                  <p className="text-[15px] font-semibold text-white">
                    {formatCurrency(lastResult.position_value)}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-[#707990] mb-1">Riesgo Total</p>
                  <p className="text-[15px] font-semibold text-white">
                    {formatCurrency(lastResult.risk_amount)}{" "}
                    <span className="text-[#707990] text-[12px]">
                      ({lastResult.risk_percent}%)
                    </span>
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-[#707990] mb-1">Risk/Share</p>
                  <p className="text-[15px] font-semibold text-white">
                    ${lastResult.risk_per_share.toFixed(4)}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-[#707990] mb-1">Target 2:1</p>
                  <p className="text-[15px] font-semibold text-[#00a449]">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#2d3139]">
          <a
            href="/calculator"
            className="text-[13px] text-[#707990] hover:text-[#00a449] transition-colors"
          >
          ...
          <button
            ...
            className="px-6 py-2.5 bg-[#00a449] hover:bg-[#00a449]/90 disabled:bg-[#2d3139] disabled:text-[#707990] text-white font-medium text-[14px] rounded transition-colors"
          >
